...
const newslopeModal = new bootstrap.Modal(newslopeModalEl);
...
map.on('dblclick', function(event){
    ...
    var text = "Ajouter une nouvelle pente au point de coordonnées " + event.latlng.lat + " / " + event.latlng.lng + " ? ";
    if (confirm(text)) {
        document.getElementById('newslope_lat').value = event.latlng.lat;
        document.getElementById('newslope_lng').value = event.latlng.lng;

        // Auto remplissage du département et pays
        fetch('https://nominatim.openstreetmap.org/reverse?format=json&zoom=10&lat=' + event.latlng.lat + '&lon=' + event.latlng.lng)
            .then(response => {
            ...
            .then(data => {
                ...
                if (country === 'France') {
                    ...
                    document.getElementById("newslope_dept").value = data['address']['state'];
                }
                document.getElementById("newslope_country").value = country;
            })
            .catch(error => {
...
function showFlash(message, type = 'success') {
    ...
    document.getElementById('flashContainer').insertAdjacentHTML('beforeend', html);
