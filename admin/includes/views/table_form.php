...
<form method="post" action="<?= admin_url('table.php?t=' . urlencode($tableKey)) ?>" enctype="multipart/form-data" class="rcs-card p-3 p-md-4">
        ...
        <?php foreach ($schema['columns'] as $colName => $colDef): ?>
            ...
            <div class="col-12 <?= $isWide ? '' : 'col-md-6' ?>">
                ...
                <?php switch ($type):
                    ...
                    case 'image_upload': ?>
                        <?php if (!empty($value)): ?>
                            <div class="rcs-current-image mb-2">
                                <img src="<?= e((string) $value) ?>" alt="" class="rcs-image-thumb" style="max-height:100px;">
                                <div class="form-text mb-0">Image actuelle — sélectionnez un fichier ci-dessous pour la remplacer.</div>
                            </div>
                        <?php endif; ?>
                        <input type="file" class="form-control <?= $hasError ? 'is-invalid' : '' ?>"
                               id="field_<?= e($colName) ?>" name="<?= e($colName) ?>"
                               accept="image/jpeg,image/png,image/gif,image/webp"
                            <?= (!empty($colDef['required']) && empty($value)) ? 'required' : '' ?>>
                        <?php break;
